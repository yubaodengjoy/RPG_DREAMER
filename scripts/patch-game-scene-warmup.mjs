...
function upgradeWarmupV2(file, source) {
  ...
  const battleWarmup = `function __rpgWarmBattles(e,t){let n=${registry}(),r=n.npcs.filter(e=>e.scene===t&&e.battle).map(e=>e.battle);r.forEach(t=>${battleLoader}(e,t)),r.length&&(e.load.isLoading()?e.load.update():e.load.start())}function __rpgWarmReachable(e,t){return __rpgWarmBattles(e,t),__rpgWarmAdjacent(e,t)}`;
...
function patchBundle(file) {
  ...
  const battleHelpers = `function __rpgWarmBattles(e,t){let n=${registry}(),r=n.npcs.filter(e=>e.scene===t&&e.battle).map(e=>e.battle);r.forEach(t=>${battleLoader}(e,t)),r.length&&(e.load.isLoading()?e.load.update():e.load.start())}function __rpgWarmReachable(e,t){return __rpgWarmBattles(e,t),__rpgWarmAdjacent(e,t)}`;
